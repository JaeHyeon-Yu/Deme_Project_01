...
D3D12_SHADER_BYTECODE CShader::CompileShaderFromFile(LPCWSTR pszFileName, LPCSTR pszShaderName,
	LPCSTR pszShaderProfile, ID3DBlob** ppd3dShaderBlob)
{
	...
	D3DCompileFromFile(pszFileName, NULL, NULL, pszShaderName, pszShaderProfile,
		nCompileFlags, 0, ppd3dShaderBlob, &pd3dErrorBlob);//쉐이더 코드의 오류로 컴파일 불가. 하지만 그렇다기엔 기존의 쉐이더 코드도 사용불가
...
void CObjectsShader::BuildObjects(ID3D12Device* pd3dDevice, ID3D12GraphicsCommandList* pd3dCommandList)
{
	...
	/*x축, y축, z축 양의 방향의 객체 개수이다. 각 값을 1씩 늘리거나 줄이면서 실행할 때
	프레임 레이트가 어떻게 변하는 가를 살펴보기 바란다.*/
	int xObjects = 3, yObjects = 3, zObjects = 3, i = 0;

	//x축, y축, z축으로 21개씩 총 21x21x21=9261개의 정육면체를 생성하고 배치한다.
