...
protected:
	...
	//카메라의 로컬 x축,y축,z축을 나타내는 벡터이다.
	XMFLOAT3 m_xmf3Right;
	XMFLOAT3 m_xmf3Up;
	XMFLOAT3 m_xmf3Look;

	//카메라가 x축,y축,z축으로 얼만큼 회전했는지를 나타내는 벡터이다.
	float m_fPitch;
	float m_fYaw;
	float m_fRoll;

	//카메라의 종류(1인칭 카메라,스페이스쉽 카메라,3인칭 카메라)를 나타낸다.
	DWORD m_nMode;
...
public:
	...
	/*카메라가 여러번 회전을 하게 되면 누적된 실수 연산의 부정확성 때문에 카메라의 로컬 x축,y축,z축이 서로 직교하지 않을 수 있다.
	카메라의 로컬 x축,y축 z축이 서로 직교하도록 만들어준다.*/
	...
	//카메라를 x축,y축,z축으로 회전하는 가상함수이다.
	virtual void Rotate(float fPitch = 0.0f, float fYaw = 0.0f, float fRoll = 0.0f){}
	//카메라의 이동, 회전에 따라 카메라의 정보를 갱신하는 가상함수이다.
	virtual void Update(XMFLOAT3& xmfLookAt, float fTimeElapsed){}
